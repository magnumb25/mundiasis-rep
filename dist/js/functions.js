...
$(document).on('click', '#tab_default_1_choice', function() {
  var msgDiv = document.getElementById('message');
  msgDiv.innerHTML='';
  msgDiv.className='';
  ...
  document.getElementById("marksFile").innerHTML = '<a href="indexx.php?page=uploadFile&id_cours='+id_cours+'" class="btn btn-primary"><i class="fa fa-upload"></i> Importer les notes</a>&nbsp;&nbsp;<a href="indexx.php?page=dmf&id_cours='+id_cours+'" class="btn btn-primary"><i class="fa fa-download"></i> Fichier de notes</a>';
...
// Rattrapage
$(document).on('click', '#tab_default_2_choice', function() {
  var msgDiv = document.getElementById('message');
  msgDiv.innerHTML='';
  msgDiv.className='';

  document.getElementById("marksFile").innerHTML = '';


  var id_cours = $('#coursOfConnectedProf').val();

  getStudentHavingRatrappage(id_cours);
  

  localStorage.setItem("id_cours", id_cours);
  showPrintButton_p2(id_cours);
});



function getStudentHavingRatrappage(id_cours){
  $.ajax({
    url: 'ajax.php?action=getStudentHavingRatrappage&id_cours='+id_cours,
    type:'GET',
    dataType: 'json',
    success: function(data){
      var pourc = $.parseJSON(data['pourc']);
      var content="";

      if (pourc.pourcentageDone < 100) {
        $('#ListOfStudentHavingRatrappage').html('\
          <center>\
          <h2>\
          Le pourcentage total des épreuves passées pour ce cours doit etre égal à 100%\
          </h2>\
          </center>\
          ');
      }
      else if (pourc.pourcentageDone == 100) {
        $('#ListOfStudentHavingRatrappage').html('');

        if (data['studRatra'].length == 0) {
          $('#ListOfStudentHavingRatrappage').html('\
            <center>\
            <h2>\
            Aucun étudiant n\'a de rattrappage pour ce cours !\
            </h2>\
            </center>\
            ');
        }
          ...
          content = '\
          <table id="listOfStudentHavingRatr" class="table table-responsive table-bordered table-striped table-hover table-condensed">\
          <thead>\
          <th class="bg-danger"><h4>Nom Complet</h4></th>\
          </thead>\
          <tbody>';

          var tmp = "";
          for (var i = 0; i <= data['studRatra'].length - 1; i++) {
            tmp = '\
            <tr>\
            <td>'+data['studRatra'][i].nom_etudiant.toUpperCase() + " " + data['studRatra'][i].prenom_etudiant.toUpperCase() + '</td>\
            </tr>\
            ';
            content = content + tmp;
          }
          ...
          $('#ListOfStudentHavingRatrappage').html(content);
          $('#ListOfStudentHavingRatrappage').className ="box-body col-md-10";


          // details ratra

          //var infoRatra = $.parseJSON(data);

          console.log(data['infoRatra']);
          var tableau = document.getElementById("listOfStudentHavingRatr");
          var arrayLignes = tableau.rows; 
          var longueur = arrayLignes.length;
          ...
          for (var j = 0; j < longueur; j++) {
            ...
            if (j==0) {
              //console.log("tmp2");
              cell.className ="bg-danger";
              //cell.innerHTML = 'ffffffffffff';
              cell.innerHTML += '<th>\
              <center>\
              <button type="button" class="btn btn-primary noPrint" id="btn-modifyTestRatra"><i class="fa fa-pencil-square-o"></i></button>\
              </center>\
              </th>\
              <th><center><h4 class="unselectable">Examen de rattrapage</h4></center></th>';
            }
            else{
              cell.innerHTML += '<center>'+data['infoRatra'][(j-1)].note_ratrapage+'</center>';
            }
          }
        }
      }
...
$(document.body).on('click', '#btn-modifyTestRatra',  function() {
  var id_cours = $('#coursOfConnectedProf').val();

  getTableDataRattrapage(id_cours);
  //disableButtons();
});

// edit Mode
function getTableDataRattrapage(id_cours){
  $.ajax({
    url: 'ajax.php?action=getAllInfoAbtoutThisRatra&id_cours='+id_cours,
    type: 'GET',
    success : function(data){
        //console.log(data);
        var json_obj = $.parseJSON(data);
        //console.log(json_obj);
        var json_infoRatra = json_obj.noteRatra;
        //var json_noteTest = json_obj.noteOfThatTest;
        //console.log(json_infoRatra);
        ...
        for (var j = 0; j < longueur; j++) {
          ...
          if (j==0) {
            ...
            cell.innerHTML += '<th><center><h4 class="unselectable">Examen de rattrapage</h4></center></th>';
          }
          else{
            cell.innerHTML += '<div class="row"><div class="col-xs-8 col-md-offset-2"><input id="note" name="noteStud'+j+'" type="number" placeholder="" min="0" max="20" step="0.01" class="form-control input-md input_mod" value="'+json_infoRatra[(j-1)].note_ratrapage+'"></div></div>';
          }
        }
        var ligneFoot = tableau.insertRow(-1);
        ligneFoot.insertCell(0);
        
        var cellFoot = ligneFoot.insertCell(1);
        cellFoot.innerHTML += '<input type="hidden" name="code_type_test"><center><button id="btn-submitNoteRatraStudent" class="btn btn-success"><i class="fa fa-floppy-o"></i>   Enregistrer</button></center>';
...
// #btn-submitNoteStudent to Database
$(document.body).on('click', '#btn-submitNoteRatraStudent',  function() {
  console.log("Submit-Test-1");

  var selectedCourse = document.getElementById('coursOfConnectedProf').value;
  var queryString = $("#form-affectNotesToStudent2").serialize()+'&action=submitNoteRatraStudent';
  var msgDiv = document.getElementById('message');
...
function showPrintButton_p2(id_cours){
  var div = document.getElementById("p_printDiv2");
  if (id_cours=="") {
    div.innerHTML = '';
  }
  else{
...
// Final Result
$(document).on('click', '#tab_default_3_choice', function() {
  var msgDiv = document.getElementById('message');
  msgDiv.innerHTML='';
  msgDiv.className='';
  ...
  generateResult(id_cours);

  localStorage.setItem("id_cours", id_cours);
  showPrintButton_p3(id_cours);
});

function showPrintButton_p3(id_cours){
  var div = document.getElementById("p_printDiv3");
  if (id_cours=="") {
    div.innerHTML = '';
  }
  else{
...
function generateResult(id_cours){
  $.ajax({
    url: 'ajax.php?action=getStudentFollowingThisCourse&id_cours='+id_cours,
    type:'GET',
    dataType: 'json',
    success: function(data){
      ...
      if (data[0].length == 0) {
        ...
        content = '\
        <table id="finalResult" class="table table-responsive table-bordered table-striped table-hover table-condensed">\
        <thead>\
        <th class="bg-info"><h4>Nom Complet</h4></th>\
        </thead>\
        <tbody>';
        ...
        $('#FinalResult').className ="box-body col-md-10";

        // Traitement des tests
        if (json_infoTest.length > 0) {
          // ajouter header of Test
          for (var i = 0; i < json_infoTest.length; i++) {
            var tableau = document.getElementById("finalResult");
            var arrayLignes = tableau.rows; 
            var longueur = arrayLignes.length;
            var tmp = 0;
            var code_type_test = json_infoTest[i].code_type_test;
              ...
              success : function(data){
                ...
                for (var j = 0; j < longueur; j++) {
                  ...
                  if (j==0) {
                    ...
                    cell.className ="bg-info";
                    cell.innerHTML += '<th>\
                    <center><h4 class="unselectable">'+json_infoTest[0].libelle_type_test+" - "+json_infoTest[0].pourcentage_test+"% "+'</h4></center></th>';
                  }
                  else{
                    cell.innerHTML += '<center>'+json_noteTest[(j-1)].note+'</center>';
                  }
            ...
            success: function(data){
              ...
              if (json.restePourcentage == 0) {
                ...
                for (var j = 0; j < longueur; j++) {
                  ...
                  if (j==0) {
                    cell.className ="bg-info";
                    cell.innerHTML += '<th><center><h4 class="unselectable"><strong>Moyenne</strong></h4></th>';
                  }
                  else{
                    if (json.moyenne[j-1] < 10) {
                      cell.className ="bg-danger";
                  ...
                  success : function(data){
                    ...
                    for (var j = 0; j < longueur; j++) {
                      ...
                      if (j==0) {
                        ...
                        if (id_ratra.includes(j)) {
                          cell.innerHTML += '<div class="row"><div class="col-xs-8 col-md-offset-2"><center>'+ json_infoRatra[tmp].note_ratrapage +'</center></div></div>';
                          tmp++;
                        }
                        else
                ...
                // afficher moyenne globale
                $.ajax({
                  url: 'ajax.php?action=getmoyenneGlob&id_cours='+id_cours,
                  type: 'GET',
                  success : function(data){
                    var json_obj = $.parseJSON(data);
                    var json_moyenne = json_obj.moyenne;
                    ...
                    for (var j = 0; j < longueur; j++) {
                      ...
                      if (j==0) {
                        cell.className ="bg-info";
                        cell.innerHTML += '<th><center><h4 class="unselectable">Moyenne générale</h4></center></th>';
                      }
                      else{
                        if (json_moyenne[j-1]>10) {
                          cell.className ="bg-success";
                        }
                        else
                          cell.className ="bg-danger";
                        cell.innerHTML += '<div class="row"><div class="col-xs-8 col-md-offset-2"><center>'+ json_moyenne[j-1]+'</center></div></div>';
