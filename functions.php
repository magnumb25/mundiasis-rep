...
function getStudentHavingRatrappage($id_cours){
	...
	$test_pourc = getPourcentageOfTestAlreadyPassed($id_cours);

	$emailsStudRatra = array();
	$studRatra = array();


	$emailsStudRatra = getidStudentHavingRatrappage($id_cours);
	foreach ($emailsStudRatra as $email) {

		$tmp_ratra = checkIfRatraAlreadyDefined($id_cours, $email);

		if ($tmp_ratra == 0) {
			$note = null;

			$add1 = "INSERT INTO `note_rattrapage` (`code_cours`, `email_etudiant`, `note_ratrapage`) 
			VALUES('$id_cours','$email','$note')";
			$stmt1 = $bdd->prepare($add1);
			$stmt1->execute();
		}
		...
		array_push($studRatra, $r);
	}

	$query1 = "SELECT *	FROM note_rattrapage, etudiant WHERE etudiant.email_etudiant=note_rattrapage.email_etudiant AND code_cours='$id_cours' ORDER BY nom_etudiant ASC";
	$q1 = $bdd->query($query1);
	$r1 = $q1->fetchall();

	return json_encode(['pourc' => $test_pourc, 'studRatra' => $studRatra, 'infoRatra' => $r1]);
}

function checkIfRatraAlreadyDefined($code_cours, $etudiant){
	global $bdd;

	$query = "SELECT * FROM note_rattrapage WHERE code_cours='$code_cours' AND email_etudiant='$etudiant'";
	$q = $bdd->query($query);
	$count = $q->rowCount();
	return $count;
}

function getidStudentHavingRatrappage($id_cours){
	global $bdd;
	...
	if ($restePourcentage == 0) {
		...
		while ($stud = $q1->fetch()) {
			...
			if ($tmp < 10) {
				array_push($moyenne, round($tmp, 2));
				array_push($stud_array, $etudiant);
			}
		}
	}
...
function getAllInfoAbtoutThisRatra($id_cours){
	global $bdd;
	...
	return json_encode(['noteRatra' => $r1]);
}


function addNoteRatraStudentToDatabase($selectedCourse){
	global $bdd;

	// Get students Following the course

	$query = "SELECT *	FROM note_rattrapage, etudiant WHERE etudiant.email_etudiant=note_rattrapage.email_etudiant AND code_cours='$selectedCourse' ORDER BY nom_etudiant ASC";
	$q = $bdd->query($query);
	$tmp = 1;
	while($r = $q->fetch()){
		$emailofThatStudent = $r['email_etudiant'];
		...
		if (!isset($noteOfThatStudent) || $noteOfThatStudent == "") {
			...
			if ($noteOfThatStudent > 20 || $noteOfThatStudent < 0) {
				...
				try {
					...
					$qrADD = "UPDATE note_rattrapage 
					SET note_ratrapage='$noteOfThatStudent'
					WHERE code_cours='$selectedCourse'
					AND email_etudiant='$emailofThatStudent'";
...
function getmoyenneGlob($id_cours){
	global $bdd;
	...
	if ($restePourcentage == 0) {
		...
		while ($stud = $q1->fetch()) {
			...
			if ($moy > 10) {
				...
				$qq2 = $bdd->query($query2);



				$query4 = "SELECT note_ratrapage FROM note_rattrapage
				WHERE code_cours = '$id_cours'
				AND email_etudiant='$etudiant'";
				$q4 = $bdd->query($query4);
				$r4 = $q4->fetch();
				$note_ratt = $r4['note_ratrapage'];


				while ($typeTT = $qq2->fetch()) {

					$code_test2 = $typeTT['code_type_test'];
					$porc_test2 = $typeTT['pourcentage_test'];

					$query3 = "SELECT note FROM note
					WHERE code_cours = '$id_cours'
					AND code_type_test='$code_test2'
					AND email_etudiant='$etudiant'";
					$q3 = $bdd->query($query3);
					$r3 = $q3->fetch();
					$note2 = $r3['note'];
					...
					$tmp2 += $note2 * ($porc_test2 / 100 );
				}

				$finalMoy = $tmp2;
				array_push($moyenne, $finalMoy);
			}		
		}
	}
 ...
 //print_r(getmoyenneGlob('CPT4202.2016'));
?>
